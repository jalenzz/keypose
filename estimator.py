...
def est_model_fn(features, labels, mode, params):
    ...
    tf.compat.v1.summary.image(
        "viz_img_L", ls.add_keypoints(features["img_L"], preds["uvd"]), max_outputs=4
    )
    tf.compat.v1.summary.image(
        "viz_probs",
    ...
    tf.compat.v1.summary.scalar(
        "0_MAE_world", tf.reduce_mean(ls.world_error(labels, xyzw))
    )
    tf.compat.v1.summary.scalar(
        "MAE_2cm", tf.reduce_mean(ls.lt_2cm_error(labels, xyzw))
    )
