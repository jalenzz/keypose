...
def keypoint_loss_targets(uvd, keys_uvd, mparams):
    ...
    print("keys_uvd shape in klt [batch, order, num_targs, 4, num_kp]:", keys_uvd.shape)
...
def keypose_loss_proj(uvdw_pos, labels, mparams, num_order):
    ...
    to_world_order = tf.stack([to_world] * num_order, axis=1)  # [batch, order, 4, 4]
    to_world_order = tf.ensure_shape(
        to_world_order, [None, num_order, 4, 4], name="to_world_order"
    )
    world_coords = project(to_world_order, uvdw_pos, True)  # [batch, order, 4, num_kp]
    ...
    print(
        "Model fn targets_to_uvd shape [batch, num_targs, 4, 4]:", targets_to_uvd.shape
    )
    print(
        "Model fn targets_keys_uvd_order shape [batch, order, num_targs, 4, num_kp]:",
        targets_keys_uvd_order.shape,
    )
    ...
    print("proj_uvds shape [batch, order, num_targs, 4, num_kp]:", proj_uvds.shape)
...
def keypose_loss_kp(uvdw_order, keys_uvd_order, mparams):
    ...
    ret = tf.reduce_sum(
        tf.square(uvdw_order[..., :3] - keys_uvd_order[..., :3]), axis=[-1, -2]
    )  # [batch, order]
    return ret
...
def keypose_loss_prob(prob_order, prob_label_order):
    ret = tf.reduce_sum(prob_order * prob_label_order, axis=[-1, -2])  # [batch, order]
...
def adjust_proj_factor(step, loss_step, minf=0.0):
    ...
    return tf.maximum(
        minf,
        tf.minimum(
            (step - loss_step[0]) / tf.cast(loss_step[1] - loss_step[0], tf.float32),
            1.0,
        ),
    )
...
def keypose_loss(labels, preds, step, mparams, do_print=True):
    ...
    loss_kp = keypose_loss_kp(uvdw_order, keys_uvd_order, mparams)  # [batch, order]
...
def add_keypoints(img, uv, colors=None):
    ...
    return tf.image.draw_bounding_boxes(img, tf.cast(keys_bb, dtype=tf.float32), colors)
...
def lt_2cm_error(labels, xyzw):
    ...
    return (
        100.0
        * tf.cast(tf.math.count_nonzero(lt, axis=[-1]), tf.float32)
        / tf.cast(tf.shape(err)[1], tf.float32)
    )  # [batch]
