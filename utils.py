...
def get_keypoints(targ_pb):
    ...
    for kp_pb in targ_pb.keypoints:
        uvds.append(np.array([kp_pb.u, kp_pb.v, focal_length * baseline / kp_pb.z, 1]))
        visible.append(kp_pb.visible)
    transform = get_transform(targ_pb.transform)
    if transform is None:
        world_t_uvd = None
    else:
        world_t_uvd = np.linalg.inv(transform).dot(q_matrix_from_camera(targ_pb.camera))
...
def get_params(
    param_file=None, cam_file=None, cam_image_file=None, defaults=DEFAULT_PARAMS
):
...
def rotation_transform(x_rot, y_rot, z_rot):
    ...
    xt = np.array(
        [
            [1.0, 0, 0, 0],
            [0, np.cos(x_rot), -np.sin(x_rot), 0.0],
            [0, np.sin(x_rot), np.cos(x_rot), 0.0],
            [0, 0, 0, 1],
        ]
    )
...
def rotate_camera(rotation, image, camera, transform, key_pts):
    ...
    def in_bounds(pt, bounds):
        if pt[0] >= 0 and pt[0] <= bounds[0] and pt[1] >= 0 and pt[1] <= bounds[1]:
            return 1.0
        else:
            return 0.0
    ...
    key_pts_p = project_np(np.dot(pmat, np.dot(rotation, qmat)), np.transpose(key_pts))
...
def do_rotation(image, image2, transform, camera, key_pts, visible, rotation):
    ...
    while True:
        ...
        if float(area_p) / area > 0.6:
            if image2 is not None:
                image2_p, _, _, _ = rotate_camera(
                    rot, image2, camera, transform, key_pts
                )
            else:
                image2_p = image_p
            break
    ...
    return (
        ...
        image2_p.astype(np.float32),
        transform_p.astype(np.float32),
        key_pts_p.astype(np.float32),
...
def do_2d_homography(image, image2, scale, shear, flip, mirrored, split):
    ...
    hom = warp_homography(
        ...
        [random.uniform(-shear[0], shear[0]), random.uniform(-shear[1], shear[1])],
        flip,
    )
...
def gauss2d(mean, var, size):
    ...
    return np.float32(
        ...
        / (2.0 * np.pi * vx * vy)
        * np.exp(
            -((x - mx) ** 2.0 / (2.0 * vx**2.0) + (y - my) ** 2.0 / (2.0 * vy**2.0))
        )
    )
...
def do_composite(image, bg_fname, sigma, motion, noise, gamma):
    ...
    def make_random(x):
        ...
        assert isinstance(x, np.ndarray), (
            "Argument to do_composite must be list or array"
        )
        if x.size == 0:
            return None
        elif x.size == 1:
    ...
    if noise is not None:
        image[:, :, :3] += np.random.randn(ys, xs, 3) * noise * 0.5 / 255.0
    ...
    if noise is not None:
        im[:, :, :3] += np.random.randn(ys, xs, 3) * noise * 0.5 / 255.0
    np.clip(im, 0.0, 1.0, im)
...
def do_occlude_crop(
    image, image2, key_pts, key_pts_r, crop, visible, dither, var_offset=False
):
...
class MeshObj:
    ...
    def make_reduced(self, num):
        ...
        self.xyzw_reduced = np.concatenate(
            [self.vertices_reduced, np.ones((self.vertices_reduced.shape[0], 1))],
            axis=-1,
        )
    ...
    def _make_colored_subset(self, num):
        ...
        self.reduced_colors = (
            255 * colors(np.interp(zs, (zs.min(), zs.max()), (0, 1)))[:, :3]
        ).astype(np.uint8)
        ...
        for i, pt in enumerate(self.uvds_reduced):
            ...
            if self.large_points:
                if (
                    u > 0
                    and v > 0
                    and u < image.shape[1] - 1
                    and v < image.shape[0] - 1
                ):
                    for ui in range(-1, 2):
                        for vi in range(-1, 2):
...
def read_mesh(path, num=300):
    ...
    print(
        "Obj file has %d vertices and %d keypoints"
