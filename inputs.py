...
def filter_nonvisible(_, labels):
    ...
    isallowed = tf.equal(visible, tf.constant(0.0))
    reduced = tf.reduce_sum(tf.cast(isallowed, tf.float32))
    visible_r = labels["visible_R"]
    isallowed_r = tf.equal(visible_r, tf.constant(0.0))
    reduced_r = tf.reduce_sum(tf.cast(isallowed_r, tf.float32))
    return tf.equal(tf.add(reduced, reduced_r), tf.constant(0.0))
...
def parser(serialized_example, resx, resy, num_kp, sym=None, return_float=True):
    ...
    fs = tf.io.parse_single_example(
        ...
        features={
            ...
            "to_world_L": tf.io.FixedLenFeature([4, 4], tf.float32),
            "to_world_R": tf.io.FixedLenFeature([4, 4], tf.float32),
            "to_uvd_L": tf.io.FixedLenFeature([4, 4], tf.float32),
            "to_uvd_R": tf.io.FixedLenFeature([4, 4], tf.float32),
            "camera_L": tf.io.FixedLenFeature([7], tf.float32),
            "camera_R": tf.io.FixedLenFeature([7], tf.float32),
            "keys_uvd_L": tf.io.FixedLenFeature([num_kp, 4], tf.float32),
            "keys_uvd_R": tf.io.FixedLenFeature([num_kp, 4], tf.float32),
            "visible_L": tf.io.FixedLenFeature([num_kp], tf.float32),
            "visible_R": tf.io.FixedLenFeature([num_kp], tf.float32),
            "num_kp_L": tf.io.FixedLenFeature([], tf.int64),
            "num_kp_R": tf.io.FixedLenFeature([], tf.int64),
            "num_targets_L": tf.io.FixedLenFeature([], tf.int64),
            "num_targets_R": tf.io.FixedLenFeature([], tf.int64),
            "mirrored": tf.io.FixedLenFeature([], tf.int64),
            "targets_to_uvd_L": tf.io.FixedLenFeature(
                [utils.MAX_TARGET_FRAMES, 4, 4], tf.float32
            ),
            "targets_to_uvd_R": tf.io.FixedLenFeature(
                [utils.MAX_TARGET_FRAMES, 4, 4], tf.float32
            ),
            "targets_keys_uvd_L": tf.io.FixedLenFeature(
                [utils.MAX_TARGET_FRAMES, num_kp, 4], tf.float32
            ),
            "targets_keys_uvd_R": tf.io.FixedLenFeature(
                [utils.MAX_TARGET_FRAMES, num_kp, 4], tf.float32
            ),
        },
    )
    ...
    if len(sym) > 1:
        ...
        mask3 = tf.stack(
            [mask2] * utils.MAX_TARGET_FRAMES, axis=0
        )  # [targets, order, 1, 1]
        fs["keys_uvd_L"] = permute_order(fs["keys_uvd_L"], order, mask2)
        fs["keys_uvd_R"] = permute_order(fs["keys_uvd_R"], order, mask2)
        fs["targets_keys_uvd_L"] = permute_order(
            fs["targets_keys_uvd_L"], order, mask3, True
        )
        fs["targets_keys_uvd_R"] = permute_order(
            fs["targets_keys_uvd_R"], order, mask3, True
        )
    print("Targets to_uvd shape [num_targs, 4, 4]:", fs["targets_to_uvd_L"].shape)
    print(
        "Targets keys_uvd shape [num_targs, num_kp, 4]:", fs["targets_keys_uvd_L"].shape
    )
...
def create_input_fn(
    params, split="val", keep_order=False, filenames=None, late_fusion=False
):
