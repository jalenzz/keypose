...
def predict(model_dir, image_dir, params, camera, camera_input, mesh_file=None):
    ...
    for fname in filenames:
        ...
        im_l = utils.resize_image(
            im_l, camera, camera_input, targs_pb
        )  # NB: changes targs_pb.
        ...
        kps_pb = targs_pb.kp_target
        im_r = utils.resize_image(
            im_r, camera, camera_input, targs_pb
        )  # NB: changes kps_pb.
        ...
        t0 = time.time()
        preds = predict_fn(
            img_L=img_l, img_R=img_r, to_world_L=to_world_l, offsets=offsets, hom=hom
        )
        ...
        im_large = cv2.vconcat(
            [cv2.hconcat([iml_orig, imr_orig]), cv2.hconcat([im_kps, im_mesh])]
        )
        if mesh_file:
    ...
    print(
        "Total time: %.1f for %d inferences, %.1f ms/inf"
...
def main(argv):
    ...
    predict(
        model_dir, image_dir, params, camera_model, camera_image, mesh_file=mesh_file
    )
