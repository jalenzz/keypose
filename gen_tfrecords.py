    ...
    for image_dir in image_dirs:
        ...
        while j < total:
            ...
            generate(
                ...
                cam,
                filenames_left[j:jj],
                filenames_right[j:jj],
                filenames_target_left[j:jj],
                filenames_target_right[j:jj],
            )
            j = jj
    ...
    with tf.io.TFRecordWriter(path) as tfrecord_writer:
        with tf.Graph().as_default():
            ...
            with tf.compat.v1.Session() as sess:
                for fleft, fright, ftleft, ftright in zip(
                    fnames_left, fnames_right, fnames_target_left, fnames_target_right
                ):
                    assert os.path.basename(
                        os.path.splitext(fleft)[0]
                    ) == os.path.basename(os.path.splitext(ftleft)[0])
                    assert os.path.basename(
                        os.path.splitext(fright)[0]
                    ) == os.path.basename(os.path.splitext(ftright)[0])
                    print(fleft)
                    ...
                    image_left = utils.resize_image(
                        image_left, cam_new, cam_orig, targs_left
                    )
                    image_right = utils.resize_image(
                        image_right, cam_new, cam_orig, targs_right
                    )
                    st0, st1 = sess.run(
                        [encoded0, encoded1],
                        feed_dict={im0: image_left, im1: image_right},
                    )
...
def make_features(targs_pb, pf):
    ...
    proj_targs = [utils.get_contents_pb(targ_pb) for targ_pb in targs_pb.proj_targets][
    ...
    feats = {
        "to_world_" + pf: feat_floats(to_world.flatten()),
        "to_uvd_" + pf: feat_floats(to_uvd.flatten()),
        "camera_" + pf: feat_floats(utils.cam_pb_to_array(camera)),
        "keys_uvd_" + pf: feat_floats(np.array(keys_uvd).flatten()),
        "visible_" + pf: feat_floats(visible),
        "num_kp_" + pf: feat_int(num_kp),
        "num_targets_" + pf: feat_int(num_targets),
        "targets_to_uvd_" + pf: feat_floats(np.array(targets_to_uvd).flatten()),
        "targets_keys_uvd_" + pf: feat_floats(np.array(targets_keys_uvd).flatten()),
        "mirrored": feat_int(int(targs_pb.mirrored)),
    }
    return feats
...
def group_tfrecords(train_dirs, val_dirs, tfset, data_params):
    ...
    out_dir = os.path.join(utils.KEYPOSE_PATH, os.path.join("tfrecords", tfset.name))
    ...
    utils.write_to_text_file(os.path.join(records_out_dir, "tfset.pbtxt"), tfset_pb)
...
def main(argv):
    ...
    train_dirs = [
        dr for tfdir in tfset.train for dr in glob.glob(os.path.join(common_dir, tfdir))
    ]
    val_dirs = [
        dr for tfdir in tfset.val for dr in glob.glob(os.path.join(common_dir, tfdir))
    ]
    exc_dirs = [
        dr
        for tfdir in tfset.exclude
