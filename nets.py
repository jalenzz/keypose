...
def dilated_cnn(
    ...
    for i, r in enumerate([1] + seq + seq + [1]):
        ...
        net = layers.BatchNormalization(
            scale=bn_scale, epsilon=bn_epsilon, trainable=is_training, momentum=bn_decay
...
def k_reduce_mult_sum(tensor, mult, axis=None, name=None):
    ...
    return layers.Lambda(
        reduce_mult_sum, arguments={"mult": mult, "axis": axis}, name=name
    )(tensor)
...
def to_norm(mat, mparams, weight_disp=1.0):
    ...
    return tf.concat(
        [
            ...
            mat[Ellipsis, 2:3, :] * 2.0 / mparams.modelx * weight_disp,
            mat[Ellipsis, 3:4, :],
        ],
        axis=-2,
    )
...
def keypose_model(mparams, is_training):
    ...
    uv_pix_raw, uv_pix, uvdw, uvdw_pos = convert_uvd_raw(uvd, offsets, hom, mparams)
